...
from src.ml.preprocessing_iomt import load_and_prepare_binary
...
def main() -> None:
    ...
    print_status("=" * 60)
    print_status("IoMT Binary Intrusion Detection - LightGBM", level=0)
    print_status("=" * 60)

    X_train, X_test, y_train, y_test, feature_names, preproc, class_map = load_and_prepare_binary(
    ...
    print_status(
        f"Training binary LGBM on {X_train.shape[0]:,} samples, {X_train.shape[1]:,} features",
        level=1,
    )
    ...
    # Save model and preprocessor via LightGBM's native format + numpy/scikit pipelines (joblib)
